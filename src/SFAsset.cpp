...
SFAsset::SFAsset(SFASSETTYPE type, std::shared_ptr<SFWindow> window): type(type), sf_window(window) {
	...
	case SFASSET_ALIEN:
		...
		break;
	case SFASSET_COIN:
		sprite = IMG_LoadTexture(sf_window->getRenderer(), "assets/coin.gif");
		break;
	case SFASSET_WALL:
		sprite = IMG_LoadTexture(sf_window->getRenderer(), "assets/item.gif");
		break;
	}
...
void SFAsset::Down() {
	Vector2 c = *(bbox->centre) + Vector2(0.0f, -0.1f);
	if(!(c.getX() < 0)) {
		bbox->centre.reset();
		bbox->centre = make_shared<Vector2>(c);
	}
...
void SFAsset::GoNorth() {
	Vector2 c = *(bbox->centre) + Vector2(0.0f, 0.2f);
	bbox->centre.reset();
	bbox->centre = make_shared<Vector2>(c);
}
...
void SFAsset::HandleCollision() {
	...
	if(SFASSET_PROJECTILE || SFASSET_PLAYER == type || SFASSET_WALL == type) {
		IsAlive();
	}
}